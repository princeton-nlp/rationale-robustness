...
def load_and_cache_examples(args, model_params, tokenizer, evaluate=False, split="train", output_examples=False):
    ...
    if evaluate:
        ...
        tensorized_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                ...
                                all_unique_ids, all_sentence_starts, all_sentence_ends,
                                all_sentence_mask, all_evidence_labels, all_neg_rationales)
...
def train(args, model_params, train_dataset, model, tokenizer):
    ...
    neg_rationale_tr_loss, neg_rationale_logging_loss = 0.0, 0.0
    model.zero_grad()
...
#            if step == 600: break # TODO: remove
#            if step < 1370: continue # TODO: remove
            model.train()
            ...
            info_loss = outputs[2]
            lambda0 = outputs[5].mean().item()
            if args.use_neg_rationales:
                neg_rationale_loss = outputs[6]
            if args.n_gpu > 1:
                loss = loss.mean() # mean() to average on multi-gpu parallel (not distributed) training
                class_loss = class_loss.mean()
                info_loss = info_loss.mean()
                if args.use_neg_rationales:
                    neg_rationale_loss = neg_rationale_loss.mean()
            ...
            if args.use_neg_rationales:
                neg_rationale_tr_loss += neg_rationale_loss.item()
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = predict(args, model_params, model, tokenizer, eval_features, eval_dataloader, true_rationales, global_step)
                        if ("beer" in args.data_dir and best_f1 > results) or ("beer" not in args.data_dir and best_f1 < results):
                            ...
                            if wait_step == args.wait_step:
                                ...
                                stop_training = True
                if args.wandb and global_step % 20 == 0:
                    log_dict = {
                        'lr': scheduler.get_lr()[0],
                        'loss': (tr_loss - logging_loss) / args.logging_steps,
                        'class_loss': (class_tr_loss - class_logging_loss) / 20,
                        'info_loss': (info_tr_loss - info_logging_loss) / 20,
                        'lambda0': lambda0,
                        'neg_rationale_loss': (neg_rationale_tr_loss - neg_rationale_logging_loss) / 20 if args.use_neg_rationales else 0,
                    ...
                    if args.use_neg_rationales:
                        neg_rationale_logging_loss = neg_rationale_tr_loss
...
def predict(args, model_params, model, tokenizer, eval_features, eval_dataloader, true_rationales, global_step):
    all_results = []
    all_targets = []
...
#        if step < 6000: continue # TODO: remove
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        with torch.no_grad():
            inputs = {'input_ids': batch[0],
    ...
    if args.wandb:
        log_dict = {
            'rationale_f1': evidence_iou[0]['macro']['f1'],
            'task_perf': results[0],
            'sparsity_rat': average_nnz / len(eval_dataloader),
            'global_step': global_step,
        }
        wandb.log(log_dict)
    return results
...
def evaluate(args, model_params, model, tokenizer, prefix="", output_examples=False, split="val"):
    ...
    Path(full_pred_dir).mkdir(parents=True, exist_ok=True)
    sent_prob_file = os.path.join(full_pred_dir, "sentence_probabilities.txt")
    sent_prob_file_pointer = open(sent_prob_file, "w")
...
#    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    ...
    for run_no in range(args.num_evaluations):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
            ...
            with torch.no_grad():
                ...
                example_indices = batch[3]
                docids = [features[example_index.item()].doc_id for example_index in example_indices]
                if args.model_type in ['xlnet', 'xlm']:
                    inputs.update({'cls_index': batch[4],
                                   'p_mask':    batch[5]})
                inputs.update({
                ...
                hard_rationale_pred = outputs[1]
                sent_pred_prob = [r if r != -1 else 0 for r in hard_rationale_pred[0].tolist()]
                sent_prob_file_pointer.write(docids[0] + '<SEP>' + str(sent_pred_prob) + '<SEP>' + str(batch[9][0].tolist()) + '\n')
        ...
        output_ratioanale_prediction_file = os.path.join(full_pred_dir, "rationale_predictions.json")
    ...
    comprehensiveness_kl = np.average(list(entropy(all_complement_context_logits[i], all_full_context_logits[i]) for i in range(len(all_full_context_logits))))
    ...
    sufficiency_kl = np.average(list(entropy(all_rationale_context_logits[i], all_full_context_logits[i]) for i in range(len(all_full_context_logits))))

    # My comprehensiveness score;
    my_comprehensiveness_score = np.average([max(all_rationale_context_logits[i]) - max(all_complement_context_logits[i]) for i in range(len(all_full_context_logits))])
    my_comprehensiveness_entropies = [entropy(all_rationale_context_logits[i]) - entropy(all_complement_context_logits[i]) for i in range(len(all_full_context_logits))]
    my_comprehensiveness_entropy = np.average(my_comprehensiveness_entropies)
    my_comprehensiveness_kl = np.average(list(entropy(all_complement_context_logits[i], all_rationale_context_logits[i]) for i in range(len(all_full_context_logits))))
...
def main():
    ...
    # originally model_params
    parser.add_argument('--model_name_or_path', type=str, default='distilbert-base-uncased')
    parser.add_argument('--config_name', type=str, default='distilbert-base-uncased')
    ...
    parser.add_argument('--beta_norm', type=float, default=0.0)
    ...
    parser.add_argument('--soft_eval', type=bool, default=False)
    parser.add_argument('--sampled_eval', type=bool, default=False)
    ...
    model_params["max_query_length"] = args.max_query_length
    model_params["warm_start"] = args.warm_start
    model_params["semi_supervised"] = args.semi_supervised
    model_params['use_logits'] = args.use_logits
    model = model_class.from_pretrained(
        model_class,
        pretrained_model_name_or_path=model_params["model_name_or_path"],
        config=config,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        #ckpt_path = os.path.join(args.output_dir, 'best_model/pytorch_model.bin')
        ckpt_path = os.path.join(args.output_dir, 'best_model/')
        logger.info("Evaluate the following checkpoints: %s", ckpt_path)

        model = model_class.from_pretrained(model_class, pretrained_model_name_or_path=ckpt_path, config=config, model_params=model_params, force_download=True)
        model.to(args.device)
        
        # Evaluate
        result = evaluate(args, model_params, model, tokenizer, prefix='', output_examples=True, split=args.eval_split)
