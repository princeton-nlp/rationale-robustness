...
class GatedExplainer(TransformerExplainer):
    def __init__(self, config, model_params=None):
        ...
        self.fixed_mask_length = model_params["max_query_length"]
...
class GatedSentenceExplainer(TransformerExplainer):
    def __init__(self, config, model_params=None):
        ...
        self.num_sample = model_params["num_avg"]
        self.fixed_mask_length = model_params["max_query_length"] - 1  # The first SEP take a different mask weight
        self.threshold = model_params["threshold"]
        ...
        self.use_logits = model_params['use_logits']
        self.gamma = model_params.get('gamma', 1.0)
        self.use_neg_rationales = model_params.get('use_neg_rationales', False)
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, p_mask=None,
                ...
                sentence_starts=None, sentence_ends=None, sentence_mask=None, evidence_labels=None, docids=None, neg_rationales=None):
        batch_size = input_ids.shape[0]
        outputs = self.encoder(input_ids,
                               attention_mask=attention_mask,
                               head_mask=head_mask,
        ...
        if self.distribution == "binary":
            ...
            if not self.use_logits:
                ...
                Z_hat, Z_hat_fixed = self.reparameterize(logits.clone(), tau=self.tau, k=self.K, sentence_mask=sentence_mask, num_sample=self.num_sample)
#            print('p(m | x) :', p_i)
#            print('evidence_labels:', evidence_labels)
#            input()
        else:
...
#                    evidence_labels.new_ones((int(np.ceil(self.semi_supervised * batch_size)),)),
...
#                sentence_mask = sentence_mask * unsupervised_batch_mask.unsqueeze(-1)
            ...
            if self.semi_supervised > 0.0:
                ...
                total_loss += self.gamma * rationale_loss
            
            if self.use_neg_rationales:
                loss_fct_2 = nn.BCEWithLogitsLoss()
                active_loss_2 = sentence_mask.view(-1) == 1
                active_logits_2 = logits.clone().view(-1)[active_loss_2]
                active_labels_2 = (1 - neg_rationales).float().view(-1)[active_loss_2]
                #neg_rationale_loss = 1 / (loss_fct_2(active_logits_2, active_labels_2) + 1e-8)
                neg_rationale_loss = loss_fct_2(active_logits_2, active_labels_2)
                total_loss += neg_rationale_loss
                #print(active_loss)
                #print(neg_rationales)
                #print(neg_rationale_loss)
                #input()
                return total_loss, class_loss, kl_info_loss, class_logit, Z_hat, self.lambda0, neg_rationale_loss

            return total_loss, class_loss, kl_info_loss, class_logit, Z_hat, self.lambda0
        else:
...
class HardGatedSentenceExplainer(TransformerExplainer):
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, p_mask=None,
        ...
        if labels is not None:
            ...
            return total_loss, class_loss, kl_info_loss, class_logit, hard_distribution_parameters, Z_hat
        else:
