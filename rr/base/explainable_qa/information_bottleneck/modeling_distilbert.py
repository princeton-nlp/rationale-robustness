...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, head_mask=None):
        ...
        # Unlike usual masks this is a continuous approximation to a k-hot vector
        # the scores are viewed as logits (hence we need to take a safe log of the mask)
#        print(mask[0, :])
        mask = torch.log(mask)
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, labels=None):
        ...
        outputs = (probs,) + distilbert_output[1:]
...
class DistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, start_positions=None,
                end_positions=None):
        distilbert_output = self.distilbert(input_ids=input_ids,
                                            attention_mask=attention_mask,
                                            head_mask=head_mask,
