...
from evidence_utils import read_examples, convert_examples_to_features, read_json

from transformers import (WEIGHTS_NAME, BertTokenizer, RobertaTokenizer, DistilBertTokenizer, BertForSequenceClassification, \
                          BertConfig, RobertaConfig, DistilBertConfig, DistilBertForSequenceClassification, \
...
from modeling_bert import BertWithFocusedAttention, BertWithAttentionPal, BertModel
from modeling_distilbert import DistilBertWithAttentionPal, DistilBertWithFocusedAttention
from modeling_roberta import RobertaWithAttentionPal, RobertaWithFocusedAttention
...
MODEL_CLASSES = {
    ...
    'bert_focus' : (BertConfig, BertWithFocusedAttention, BertTokenizer),
    'bert_pal' : (BertConfig, BertWithAttentionPal, BertTokenizer),
    'distilbert_focus' : (DistilBertConfig, DistilBertWithFocusedAttention, DistilBertTokenizer),
    'distilbert_pal' : (DistilBertConfig, DistilBertWithAttentionPal, DistilBertTokenizer),
    'roberta_focus' : (RobertaConfig, RobertaWithFocusedAttention, RobertaTokenizer),
    'roberta_pal' : (RobertaConfig, RobertaWithAttentionPal, RobertaTokenizer),
}
...
def load_and_cache_examples(args, model_params, tokenizer, evaluate=False, split="train", output_examples=False):
    ...
    cached_features_file = os.path.join(os.path.dirname(input_file), 'cached_{}_{}_{}_fullcontext'.format(
        split,
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        features = convert_examples_to_features(args, model_params, examples=examples,
                                               tokenizer=tokenizer,
    ...
    if evaluate:
        ...
        tensorized_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_example_index, all_cls_index, all_p_mask, all_evidence_labels,
                                all_unique_ids)
    else:
        all_class_labels = torch.tensor([f.class_label for f in features], dtype=torch.long)
        tensorized_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_class_labels,  all_cls_index, all_p_mask, all_evidence_labels, all_unique_ids)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids':       batch[0],
                      ...
                      'labels': batch[3]}
            if args.model_type == 'xlm' or args.model_type == 'roberta':
                inputs.update({'token_type_ids': None})
            elif args.model_type in ['bert', 'bert_focus','bert_pal']:
                inputs.update({'token_type_ids': batch[2]})
            if args.model_type in ['xlnet', 'xlm']:
                inputs.update({'cls_index': batch[4],
                               'p_mask':       batch[5]})
            if args.focus_attention or args.pal_attention:
                inputs.update({'evidence_labels' : batch[6]})
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        logger.info("loss : %.4f" % ((tr_loss - logging_loss)/args.logging_steps))
                        if best_f1 < results:
